...
class EntityDestroyerRo(type: EntityType<out TamableAnimal>, level: Level) : EntityShipBase(type, level) {
    init {
    ...
    val passengersRidingOffset: Double
        get() {
            if (this.isInSittingPose) {
                return (this.bbHeight * 0.28f).toDouble()
            }
            return (this.bbHeight * 0.6f).toDouble()
        }
    ...
    override val shipSpawnEggItem: Item?
        get() = ModItems.DESTROYER_RO_SPAWN_EGG.get()
