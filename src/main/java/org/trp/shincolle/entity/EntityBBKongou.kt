...
class EntityBBKongou(type: EntityType<out TamableAnimal>, level: Level) : EntityShipBase(type, level) {
    init {
    ...
    private fun updateClientParticles() {
        if (this.tickCount % 4 == 0 && !this.isInSittingPose && this.getEquipFlag(EQUIP_RIGGING) && !this.isInDeadPose) {
            var partPos = CalcHelper.rotateXZByAxis(-0.7f, 0.0f, this.yBodyRot * Mth.DEG_TO_RAD, 1.0f)
            for (i in 0..2) {
            ...
            partPos = CalcHelper.rotateXZByAxis(-0.45f, 0.0f, this.yBodyRot * Mth.DEG_TO_RAD, 1.0f)
            for (i in 0..2) {
                this.level().addParticle(
                    ParticleTypes.SMOKE,
                    this.getX() + partPos[1], this.getY() + 1.32 + i * 0.1, this.getZ() + partPos[0],
                    0.0, 0.0, 0.0
                )
            }
        }
    ...
    private fun applyBuffToNearbyAllies() {
        ...
        val duration = 140 + this.getStateMinor(0)
