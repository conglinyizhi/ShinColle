...
class ModelBBHiei<T : EntityShipBase>(root: ModelPart) : ShincolleShipModel<T>() {
    ...
    private fun applyEquipVisibility(entity: T?) {
        ...
        val showRigging = entity.getEquipFlag(EntityBBHiei.EQUIP_RIGGING)
        val showHat = entity.getEquipFlag(EntityBBHiei.EQUIP_ANCHOR)
    ...
    private fun applySpecialPoseAdjustments(
        ...
        if (isSitting) {
            ...
            if (sitTick > 256) {
                ...
                val parTick = ageInTicks - ageInTicks.toInt() + (entity.tickCount % 256)
                if (parTick < 30.0f) {
                    val az = Mth.sin(parTick * 0.033f * 1.5708f)
                    setFace(3)
                    ArmLeft01.zRot = 0.1f + az * 1.8f
                    ArmLeft02.zRot = max(0.0f, 1.0f - az * 2.88f)
    ...
    companion object {
        val LAYER_LOCATION: ModelLayerLocation =
            ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(Shincolle.MODID, "bb_hiei"), "main")

        private val SITTING_TRANSLATE_Y = sittingY("ModelBBHiei")
        private val SITTING_ALT_TRANSLATE_Y = sittingAltY("ModelBBHiei")
        private val RIDING_TRANSLATE_Y = ridingY("ModelBBHiei")
        private val SNEAK_TRANSLATE_Y = sneakY("ModelBBHiei")
        private val DEAD_TRANSLATE_Y = deadY("ModelBBHiei")
        ...
        fun createBodyLayer(): LayerDefinition {
            ...
            val Hair02a = HairMain.addOrReplaceChild(
                "Hair02a",
                CubeListBuilder.create().texOffs(49, 40).mirror().addBox(-6f, -6f, 0f, 6f, 5f, 6f, CubeDeformation(0f)),
                PartPose.offsetAndRotation(-2.5f, 12f, 4.7f, 0.1222f, 0.2094f, -0.9599f)
            )

            val Hair01a = HairMain.addOrReplaceChild(
                "Hair01a",
                CubeListBuilder.create().texOffs(49, 40).addBox(0f, -6f, 0f, 6f, 5f, 6f, CubeDeformation(0f)),
                PartPose.offsetAndRotation(2.5f, 12f, 4.7f, 0.1222f, -0.2094f, 0.9599f)
            )

            val Hair01 = HairMain.addOrReplaceChild(
                "Hair01",
                CubeListBuilder.create().texOffs(84, 0).addBox(-7.5f, 0f, 0f, 15f, 6f, 7f, CubeDeformation(0f)),
                PartPose.offsetAndRotation(0f, 11.2f, 3.8f, 0.5061f, 0f, 0f)
            )
