...
public class RendererNorthernHime extends MobRenderer<EntityNorthernHime, ModelNorthernHime<EntityNorthernHime>> {
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityNorthernHime entity) {
        return TEXTURE;
    }

    @Override
    protected void scale(EntityNorthernHime entity, PoseStack poseStack, float partialTickTime) {
        float s = LegacyScale.getScale(entity, this.model);
        poseStack.scale(s, s, s);
    }

    @Override
    protected float getFlipDegrees(EntityNorthernHime entity) {
