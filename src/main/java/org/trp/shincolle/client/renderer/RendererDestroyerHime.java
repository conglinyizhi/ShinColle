...
public class RendererDestroyerHime extends MobRenderer<EntityDestroyerHime, ModelDestroyerHime<EntityDestroyerHime>> {
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityDestroyerHime entity) {
        return TEXTURE;
    }

    @Override
    protected void scale(EntityDestroyerHime entity, PoseStack poseStack, float partialTickTime) {
        float s = LegacyScale.getScale(entity, this.model);
        poseStack.scale(s, s, s);
    }

    @Override
    protected float getFlipDegrees(EntityDestroyerHime entity) {
