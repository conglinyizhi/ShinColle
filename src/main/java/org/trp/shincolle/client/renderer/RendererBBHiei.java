...
public class RendererBBHiei extends MobRenderer<EntityBBHiei, ModelBBHiei<EntityBBHiei>> {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Shincolle.MODID, "textures/entity/bb_hiei.png");
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityBBHiei entity) {
        return TEXTURE;
    }

    @Override
    protected void scale(EntityBBHiei entity, PoseStack poseStack, float partialTickTime) {
        float s = LegacyScale.getScale(entity, this.model);
        poseStack.scale(s, s, s);
    }

    @Override
    protected float getFlipDegrees(EntityBBHiei entity) {
