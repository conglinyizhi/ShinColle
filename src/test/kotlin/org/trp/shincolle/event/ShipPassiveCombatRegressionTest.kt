...
class ShipPassiveCombatRegressionTest {
    ...
    @Test
    fun passiveCombatShouldStillAdvanceOnTargetsEvenWhenFollowOwnerWouldNormallyApply() {
        val source: String = Files.readString(PASSIVE_COMBAT_SOURCE)
        val brain: String = Files.readString(SHIP_BRAIN_SOURCE)
    ...
    @Test
    fun passiveCombatShouldBeDrivenByBrainBehaviors() {
        val ship: String = Files.readString(SHIP_SOURCE)
        val supportTick: String = ship.substring(ship.indexOf("protected void tickAliveLogic()"),
