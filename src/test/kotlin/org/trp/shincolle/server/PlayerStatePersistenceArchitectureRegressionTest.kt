...
class PlayerStatePersistenceArchitectureRegressionTest {
    private val PLAYER_STATE_SERVICE: Path =
            Path.of("src/main/java/org/trp/shincolle/server/PlayerStateService.kt")
    private val DATA_ATTACHMENTS: Path =
    ...
    private val FORMATION_SERVICE_SOURCE: Path =
    ...
    private val POINTER_ITEM_CLIENT_HELPER_SOURCE: Path =
    ...
    private val SHIP_POINTER_SOURCE: Path =
    ...
    @Test
    fun playerStateServiceShouldOwnMutablePlayerAttachmentAccess() {
        val service: String = Files.readString(PLAYER_STATE_SERVICE)
    ...
    @Test
    fun eventNetworkAndMenuEntrypointsShouldUsePlayerStateService() {
        val sources = mapOf(
            EVENT_SOURCE to Files.readString(EVENT_SOURCE),
            NETWORK_SOURCE to Files.readString(NETWORK_SOURCE),
            DESK_SERVICE_SOURCE to Files.readString(DESK_SERVICE_SOURCE),
            FORMATION_SERVICE_SOURCE to Files.readString(FORMATION_SERVICE_SOURCE),
            POINTER_SERVICE_SOURCE to Files.readString(POINTER_SERVICE_SOURCE),
            TEAM_DIPLOMACY_SERVICE_SOURCE to Files.readString(TEAM_DIPLOMACY_SERVICE_SOURCE),
            TARGET_PROTECTION_SERVICE_SOURCE to Files.readString(TARGET_PROTECTION_SERVICE_SOURCE),
            SHIP_MENU_SOURCE to Files.readString(SHIP_MENU_SOURCE),
            FORMATION_MENU_SOURCE to Files.readString(FORMATION_MENU_SOURCE),
            POINTER_ITEM_SOURCE to Files.readString(POINTER_ITEM_SOURCE),
            POINTER_ITEM_CLIENT_HELPER_SOURCE to Files.readString(POINTER_ITEM_CLIENT_HELPER_SOURCE),
            SHIP_SOURCE to Files.readString(SHIP_SOURCE),
            SHIP_POINTER_SOURCE to Files.readString(SHIP_POINTER_SOURCE),
            BRAIN_AI_SOURCE to Files.readString(BRAIN_AI_SOURCE)
        )

        for ((key, value) in sources) {
            val source: String = value
            assertFalse(source.contains("getData(ModDataAttachments.ADMIRAL_DATA)")) {
                "$key should not directly read/write admiral attachments"
            }
            ...
            assertFalse(source.contains("getData(ModDataAttachments.COLLECTED_SHIPS)")) {
                "$key should not directly read/write collected ship attachments"
            }
        ...
        val events: String = sources[EVENT_SOURCE]!!
        ...
        val network: String = sources[NETWORK_SOURCE]!!
        ...
        val formationService: String = sources[FORMATION_SERVICE_SOURCE]!!
        ...
        val pointerService: String = sources[POINTER_SERVICE_SOURCE]!!
        ...
        assertFalse(sources[POINTER_ITEM_SOURCE]!!.contains("updateServerSideMode(")) {
            "Pointer item should not own server-side pointer mode reconciliation rules"
        }
        assertFalse(sources[POINTER_ITEM_SOURCE]!!.contains("clearOwnedPointerSelection(")) {
            "Pointer item should not own server-side selection clearing rules"
        }

        assertTrue(sources[SHIP_MENU_SOURCE]!!.contains("PlayerStateService.registerCollectedShip(serverPlayer, classID);")) {
            "Opening ship inventory should register collection through the service"
        }
        assertTrue(sources[FORMATION_MENU_SOURCE]!!.contains("PlayerStateService.admiralData(playerInventory.player)")) {
            "Formation menu should get player state through the service"
        }
        assertTrue(sources[POINTER_ITEM_SOURCE]!!.contains("PointerItemClientHelper")) {
            "Pointer item should delegate client-only tooltip state to the client helper"
        }
        assertTrue(sources[POINTER_ITEM_CLIENT_HELPER_SOURCE]!!.contains("PlayerStateService.admiralData(mc.player)")) {
            "Pointer item client helper should read current team through the service"
        }
        assertTrue(sources[SHIP_SOURCE]!!.contains("PlayerStateService.adjustOwnedMarriedShipCount(player, 1);")) {
            "Marriage item flow should update persistent player counters through the service"
        }
    }

    @Test
    fun mutableTeamSlotWritesShouldStayInsideAdmiralDataOrPlayerStateService() {
        val sources = mapOf(
            EVENT_SOURCE to Files.readString(EVENT_SOURCE),
            NETWORK_SOURCE to Files.readString(NETWORK_SOURCE),
            SHIP_MENU_SOURCE to Files.readString(SHIP_MENU_SOURCE),
            ...
            SHIP_POINTER_SOURCE to Files.readString(SHIP_POINTER_SOURCE),
            BRAIN_AI_SOURCE to Files.readString(BRAIN_AI_SOURCE),
            COMMANDS_SOURCE to Files.readString(COMMANDS_SOURCE)
        )

        for ((key, value) in sources) {
            val source: String = value
            assertFalse(source.contains(".setShipUUID(")) {
                "$key should not mutate team slots outside PlayerStateService"
            }
            assertFalse(source.contains(".setCurrentTeamID(")) {
                "$key should not mutate current team outside PlayerStateService"
            }
            assertFalse(source.contains(".setFormationID(")) {
                "$key should not mutate formation IDs outside PlayerStateService"
            }
            assertFalse(source.contains(".setTeamName(")) {
                "$key should not mutate team names outside PlayerStateService"
            }
            assertFalse(source.contains(".swapShips(")) {
                "$key should not swap team slots outside PlayerStateService"
            }
            assertFalse(source.contains(".assignShipToTeam(")) {
                "$key should not assign team slots outside PlayerStateService"
            }
            assertFalse(source.contains(".removeShip(")) {
                "$key should not remove team slots outside PlayerStateService"
            }
        }
    }

    @Test
    fun shipRegistrySavedDataShouldPreserveOwnerCacheFields() {
        val registry: String = Files.readString(SHIP_REGISTRY)
    ...
    @Test
    fun normalEntityUnloadShouldNotMarkShipRegistryEntryRemoved() {
    ...
    @Test
    fun playerScopedTargetAndDiplomacyDataShouldPersistAsSavedData() {
        val diplomacy: String = Files.readString(TEAM_DIPLOMACY)
        val targets: String = Files.readString(PLAYER_TARGET_LIST)
        val unattackable: String = Files.readString(UNATTACKABLE_TARGETS)
        val registry: String = Files.readString(SHIP_REGISTRY)
    ...
    @Test
    fun gameplayEntrypointsShouldUseSavedDataForTargetAndDiplomacyState() {
        val network: String = Files.readString(NETWORK_SOURCE)
        val pointerService: String = Files.readString(POINTER_SERVICE_SOURCE)
        val diplomacyService: String = Files.readString(TEAM_DIPLOMACY_SERVICE_SOURCE)
        val targetProtectionService: String = Files.readString(TARGET_PROTECTION_SERVICE_SOURCE)
        val deskMenu: String = Files.readString(DESK_MENU_SOURCE)
        val targetWrench: String = Files.readString(TARGET_WRENCH)
        val passiveCombat: String = Files.readString(PASSIVE_COMBAT)
    ...
    private fun assertSavedDataUsesServerWideStorage(source: String, dataId: String) {
