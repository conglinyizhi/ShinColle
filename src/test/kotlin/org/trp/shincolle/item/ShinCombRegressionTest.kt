...
class ShinCombRegressionTest {
    ...
    fun shinCombShouldStayRestoredAsLegacyItem() {
        val creativeTabContents = Files.readString(CREATIVE_TAB_CONTENTS_SOURCE)
        val model = Files.readString(MODEL_SOURCE)
        val enUs = Files.readString(EN_US_LANG)
        val zhCn = Files.readString(ZH_CN_LANG)

        assertTrue(creativeTabContents.contains("output.accept(ModItems.SHIN_COMB.get())"))
    ...
    companion object {
        private val CREATIVE_TAB_CONTENTS_SOURCE: Path = Path.of("src/main/java/org/trp/shincolle/init/ShinColleCreativeTabContents.kt")
