...
class FontResourceReferenceRegressionTest {
    private val FONT_JSON: Path =
        Path.of("src/main/resources/assets/shincolle/font/default.json")
    private val MISANS_TTF: Path =
        Path.of("src/main/resources/assets/shincolle/font/misans.ttf")
    private val BOOK_RENDERER_SOURCE: Path =
        Path.of("src/main/java/org/trp/shincolle/client/renderer/BookRenderer.kt")

    @Test
    fun fontDefinitionShouldKeepMiSansProviderAndMinecraftFallback() {
        val fontJson = Files.readString(FONT_JSON)
    ...
    @Test
    fun bundledMiSansFontFileShouldRemainPresent() {
    ...
    @Test
    fun bookRendererShouldKeepUsingClientMiSansTogglePath() {
        val bookRenderer = Files.readString(BOOK_RENDERER_SOURCE)
